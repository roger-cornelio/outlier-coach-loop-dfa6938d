...
import { Plus, Search, ExternalLink, Loader2, ChevronLeft, ChevronRight, Users, UserPlus } from "lucide-react";
...
// ═══════════════════════════════════════════════════════════
// Sub-component: Clientes / Leads tab
// ═══════════════════════════════════════════════════════════
            ...
            {loading ? (
              ...
              rows.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell>
                    {r.instagram ? (
                      <a href={`https://instagram.com/${r.instagram}`} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline inline-flex items-center gap-1">
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Input id="crm-nome" placeholder="Nome completo" {...form.register("nome")} />
            ...
            <div className="space-y-1.5">
              ...
              <Input id="crm-telefone" placeholder="(00) 00000-0000" value={form.watch("telefone")} onChange={(e) => form.setValue("telefone", applyPhoneMask(e.target.value), { shouldValidate: true })} />
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input id="crm-instagram" placeholder="usuario" className="pl-8" {...form.register("instagram")} />
              </div>
            </div>
            <DialogFooter>
...
// ═══════════════════════════════════════════════════════════
// Sub-component: Duplas em Provas tab
// ═══════════════════════════════════════════════════════════
function DuplasTab() {
  ...
  const fetchDuplas = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Erro ao carregar duplas: " + err.message);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        {filtered.length} dupla{filtered.length !== 1 ? "s" : ""} cadastrada{filtered.length !== 1 ? "s" : ""}
      </p>
      ...
      <div className="relative w-full">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
        <Input
          placeholder="Buscar por atleta, parceiro ou prova..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="pl-10"
        />
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Parceiro(a)</TableHead>
              <TableHead>Tel. Parceiro</TableHead>
              <TableHead>Insta Parceiro</TableHead>
                ...
                <TableRow key={r.id}>
                  <TableCell>
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{r.athlete_email}</p>
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    {r.partner_instagram ? (
                      <a href={`https://instagram.com/${r.partner_instagram.replace(/^@/, "")}`} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline inline-flex items-center gap-1">
                        @{r.partner_instagram.replace(/^@/, "")} <ExternalLink className="w-3 h-3" />
                      </a>
                    ) : "—"}
                  </TableCell>
                  <TableCell className="max-w-[200px] truncate" title={r.nome}>{r.nome}</TableCell>
                  <TableCell>
                    <Badge variant={r.race_type === "ALVO" ? "default" : "secondary"} className="text-xs">
                      {r.race_type}
                    </Badge>
                  </TableCell>
    ...
    <div className="space-y-6 w-full">
      ...
      <Tabs defaultValue="clientes" className="w-full">
        <TabsList>
          ...
          <TabsTrigger value="duplas" className="gap-2">
