...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Call the external diagnostic API
    ...
    if (!apiRes.ok) {
      ...
      console.error(`[proxy-roxcoach] External API error: ${apiRes.status} - ${errorText}`);
      return new Response(JSON.stringify({ error: `Erro na API externa: ${apiRes.status}` }), {
