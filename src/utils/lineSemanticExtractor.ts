...
// ============================================
// MERGE OVERRIDES (para gabarito editável)
// ============================================

import type { SemanticOverride } from '@/types/outlier';

/**
 * Mescla segmentos extraídos com overrides do coach.
 * Overrides têm prioridade sobre segmentos brutos.
 * Retorna segmentos finais para cálculos e display.
 */
export function mergeOverrides(
  ...
  const metricSegments = segments.filter(s => s.type !== 'movement');
  const movementSegments = segments.filter(s => s.type === 'movement');

  const overrideMap = new Map<number, SemanticOverride>();
  const addedOverrides: SemanticOverride[] = [];

  for (const o of overrides) {
    if (o.isAdded) addedOverrides.push(o);
    else overrideMap.set(o.index, o);
  }

  const result: SemanticSegment[] = [...movementSegments];

  metricSegments.forEach((seg, metricIdx) => {
    const override = overrideMap.get(metricIdx);
    if (override?.isRemoved) return; // removed
    if (override) {
      result.push({ type: override.type as SemanticType, text: override.text, startIndex: seg.startIndex });
    } else {
      result.push(seg);
    }
  });
  ...
  for (const added of addedOverrides) {
    result.push({ type: added.type as SemanticType, text: added.text, startIndex: 9999 });
  }
